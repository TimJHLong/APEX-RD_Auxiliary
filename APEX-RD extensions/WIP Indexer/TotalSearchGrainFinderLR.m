function grains = TotalSearchGrainFinderLR(imgInfo, n, omeBnds, omeStep, instr, material, options)
% Finds grain orientations by the same method as 'TotalSearchGrainFinder',
% but sacrifices speed to reduce the required RAM.
% 
% USAGE: 
%   grains = TotalSearchGrainFinderLR(imgInfo, n, omeBnds, omeStep,...
...
for ii=1:gridSize
    ...
    % after the last row in a column, move to the next column
    if rowNum==n+2
        rowNum = 1;
        colNum = colNum+1;
    end

    % after the last column in a sheet, move to the next sheet.
    if colNum==n+2
        colNum = 1;
        sheetNum = sheetNum+1;
    end

    % after the last sheet in a cube, move to the next cube
    if sheetNum==n+2
        faceNum = faceNum+1;
        sheetNum = 1;
        disp(['Primary search is ~' num2str((faceNum-1)/8*100) '% complete'])
    end
